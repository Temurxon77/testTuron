...
const headCells = [
  ...
  { id: 'category', numeric: false, disablePadding: true, label: 'Наименование категории' },
  { id: 'totalQuestions', numeric: true, disablePadding: true, label: 'Количество вопросов' },
  { id: 'time', numeric: true, disablePadding: true, label: 'Длительность (мин.)' },
  { id:  'deadlineStart', numeric: true, disablePadding: true, label: 'Назначенная дата' },
];
...
const UserTests = (props) => {
  ...
  const [comment, setComment] = React.useState("")
    ...
    function getUserTests(){
        axios({
                method: "GET",
                url: "http://localhost:8000/users/questionCategories",
                headers : {
...
const cancelTest = (e) =>{
              ...
              const body = {
                ...
                axios({
                    method: "POST",
                    url: "http://localhost:8000/questions/cancel",
                    headers : {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${localStorage.getItem("token")}`
                    },
  ...
  {isLoading ? <ColorLinearProgress /> :
    ...
    <div className={classes.root}>
      <Paper className={classes.paper}>
        ...
        <DialogTitle id="customized-dialog-title" onClose={handleClose}>
                ...
                <DialogContent>
                  ...
                  <ListItem button>
                    <ListItemText primary="Наименование категории:" />
                    <ListItemText className={classes.alignRight} primary={testDetails.questionCat} />
                  </ListItem>
                  <Divider className={classes.divider} />
                  <ListItem button>
                    <ListItemText primary="Количество вопросов:" />
                  ...
                  <Divider className={classes.divider} />
                  <ListItem button>
                    <ListItemText primary="Назначенная дата:" />
      ...
      { openCancel ?
                ...
                <DialogContent>
          <DialogContentText>
            Нажимая на кнопку "Отказаться" вы отказываетесь от сдачи тестирования. Ваш результат за этот тест будет аннулирован.
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
...
const mapStateToProps = ({ isAdmin, userData, timer, testState, router }) => ({ isAdmin, userData, timer, testState, router });
